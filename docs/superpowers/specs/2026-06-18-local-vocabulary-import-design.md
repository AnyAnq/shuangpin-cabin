...
混合练习不作为新的主模块，而是词库练习中的一个选择模式。入口放在练习页顶部的已安装词库选择器内，位于所有单独词库按钮之前：
...
- “混合”按钮与单个词库按钮处于同一组，避免放在页面右侧空位形成游离操作。
- “混合”按钮使用比普通词库按钮稍重的颜色；选中时使用绿色实底或同等强度高亮，未选中时也比普通浅色按钮更醒目。
- 单个词库选中时，“混合”按钮回到未选中但仍略重的样式。
- 没有任何已安装词库时不显示混合按钮。

混合模式行为：

- 参与范围固定为全部已安装词库，包括本地词库和在线已安装词库。
- 点击“混合”后进入词库练习，并从全部已安装词库的词条中生成练习组。
- 练习标题显示 `今日练习 · 词库练习 · 混合词库`。
- 用户仍可点击任意单个词库按钮切回单独词库练习。
...
`practiceStore.ts` 需要支持两种词库选择状态：单个词库 ID，以及混合模式。单个词库练习仍从对应 package 读取 entries；混合模式从全部已安装词库读取 entries，生成一个虚拟 `VocabularyPackageFile`，再复用现有 `buildVocabularyPracticeUnits` 生成练习组。练习引擎不关心来源，只接收生成后的 `VocabularyPracticeUnit`。
...
`tests/components/AppShell.test.ts`：

- 词库练习且有已安装词库时，词库选择器第一个按钮是“混合”。
- 混合按钮使用更重的样式并能切换到混合词库。
- 混合模式标题显示“混合词库”。
...
- 点击开始练习。
- 确认练习页显示导入词库内容。
- 安装或导入多个词库后，点击“混合”能从全部词库中生成练习内容。
...
- 练习页词库选择器提供更醒目的“混合”按钮，并位于单个词库按钮之前。
- 混合模式使用全部已安装词库生成练习组，并显示“混合词库”标题。
